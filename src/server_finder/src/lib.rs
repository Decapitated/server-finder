...
pub fn find_server(group_addr: Ipv4Addr, group_port: u16,
                   phrase: String, secret: String, toggle: Arc<AtomicBool>) -> io::Result<Option<TcpStream>> {
    ...
    let server_result: io::Result<Option<SocketAddrV4>> = loop {
        ...
        break Ok(Some(SocketAddrV4::new(ip, port)));
    };
    let server_option = server_result.unwrap();
    let server_addr = match server_option {
        Some(r) => r,
        None => return Ok(None)
    };
...
pub fn find_client(toggle: Arc<AtomicBool>,
                   ...
                   phrase: String, secret: String) -> io::Result<Option<TcpStream>> { //TcpStream
    let tcp_port: Arc<AtomicU16> = Arc::new(AtomicU16::new(0));

    let toggle_clone = toggle.clone();
    let port_clone = tcp_port.clone();
    let await_thread: JoinHandle<Option<TcpStream>> = thread::spawn(move ||{
    ...
    let client = await_thread.join().expect("should be client stream option");
    finished.store(false, Ordering::SeqCst);
    Ok(client)
}

fn await_client(secret: String, toggle: Arc<AtomicBool>, tcp_port:Arc<AtomicU16>) -> io::Result<Option<TcpStream>> {
    let socket_addr = SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 0);
    ...
    while client.is_none() && toggle.load(Ordering::SeqCst) {
        ...
        stream.set_nonblocking(false).expect("should set stream to nonblocking"); // Wait for secret.
        let mut buffer = [0; 1024];
